...
def api(order):
    ...
    if (
        len(l) != len(s)
        or len(set(l)) != len(l)
        or [i for i in range(1, len(l) + 1) if i not in l]
    ):
...
def graph():
    graphs = []
    dogs_owned = {"One Dog": 67, "Two Dogs": 26, "Three Dogs": 7}
    past_owner = {"First Time": 15, "Past Owner": 85}
    graphs.append(pie(dogs_owned))
    graphs.append(pie(past_owner, inner_radius=0.5))
...
def plotly_graph():
        ...
        dict(
            data=[
                dict(
                    ...
                    hole=0,
                    textfont={"size": 24, "family": "Overpass"},
    ...
    content = {
        ...
        "description": "Playful Entertainers never want their puppy to get bored and are in search of a playtime experience that helps their dog burn-up excess energy. They want their puppy’s playtime to be an interactive experience that is intellectually stimulating and fun for the entire family. ",
    }
    ...
    return render_template(
        "plotly.html", ids=ids, graphJSON=graphJSON, content=content
    )
